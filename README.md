...
- `WA_DM_MODE` (optionnel, defaut `off`): relais des conversations privees WhatsApp (`@c.us`). `off` ignore les DM ; `all` cree automatiquement un topic Telegram par contact (meme mecanisme que pour les groupes, mapping persiste dans `WA_GROUP_IDS`)
